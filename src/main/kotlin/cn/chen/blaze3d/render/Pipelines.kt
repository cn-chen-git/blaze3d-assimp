...
object Pipelines {
    ...
    val AI_SNIPPET: RenderPipeline.Snippet = run {
        ...
        RenderPipeline.builder(matsSnippet)
            .withVertexShader("core/blaze3d_model")
            .withFragmentShader("core/blaze3d_model")
    ...
    val AI_OPAQUE: RenderPipeline = RenderPipeline.builder(AI_SNIPPET)
        .withLocation("pipeline/blaze3d_model_opaque")
        .withShaderDefine("ALPHA_CUTOUT", 0.01f)
        .withCull(false)
        .build()
    val AI_OPAQUE_CULL: RenderPipeline = RenderPipeline.builder(AI_SNIPPET)
        .withLocation("pipeline/blaze3d_model_opaque_cull")
        .withShaderDefine("ALPHA_CUTOUT", 0.01f)
        .withCull(true)
        .build()
    val AI_TRANSLUCENT: RenderPipeline = RenderPipeline.builder(AI_SNIPPET)
        .withLocation("pipeline/blaze3d_model_translucent")
        .withColorTargetState(ColorTargetState(BlendFunction.TRANSLUCENT))
        .withCull(false)
        .withDepthStencilState(DepthStencilState(CompareOp.GREATER_THAN_OR_EQUAL, true))
        .build()
    val AI_OUTLINE: RenderPipeline = RenderPipeline.builder(AI_SNIPPET)
        .withLocation("pipeline/blaze3d_model_outline")
        .withShaderDefine("MMD_OUTLINE", 1)
        .withCull(true)
