...
class AIAnimChannel(
    ...
    val preState: AIAnimBehaviour = AIAnimBehaviour.DEFAULT,
    val postState: AIAnimBehaviour = AIAnimBehaviour.DEFAULT
) {
    ...
    private fun resolveTime(times: DoubleArray, n: Int, time: Double, pre: AIAnimBehaviour, post: AIAnimBehaviour): Double {
        val start = times[0]; val end = times[n - 1]
        if (time in start..end) return time
        val behaviour = if (time < start) pre else post
        if (behaviour == AIAnimBehaviour.LINEAR && n > 1) return if (time < start) {
            val d = times[1] - start
        ...
        if (behaviour == AIAnimBehaviour.REPEAT && end > start) {
            val len = end - start
...
class AIAnimClip(
    ...
    val meshChannels: List<AIMeshAnimChannel> = emptyList(),
    val morphChannels: List<AIMorphAnimChannel> = emptyList()
) {
...
enum class AIAnimBehaviour { DEFAULT, CONSTANT, LINEAR, REPEAT }
class AIMeshAnimChannel(val meshName: String, val times: DoubleArray, val values: IntArray) {
...
class AIMorphKey(val time: Double, val targetIndices: IntArray, val weights: FloatArray)
class AIMorphAnimChannel(val meshName: String, val keys: List<AIMorphKey>) {
