...
#moj_import <minecraft:blaze3d_model_skinning.glsl>
in vec3 Position;
in vec4 Color;
in vec2 UV0;
in ivec2 UV1;
...
void main() {
...
#ifdef MMD_OUTLINE
    vec4 outlinePos = localPos;
    outlinePos.xyz += normalize(Normal) * 0.035;
    viewPos = ModelViewMat * outlinePos;
#endif
    ...
    vertexColor = minecraft_mix_light(Light0_Direction, Light1_Direction, skinnedNormal, Color);
    toonShade = clamp(dot(normalize(skinnedNormal), normalize(Light0_Direction)) * 0.5 + 0.5, 0.0, 1.0);
